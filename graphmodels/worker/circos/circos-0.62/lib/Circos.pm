...
sub run {
	...
	if ( $OPT{'version'} ) {
		printinfo($APP_NAME,$VERSION);
		exit;
	}

	$CONF{debug_group} = join($COMMA,grep($_, "conf",$OPT{debug_group}));

	printdebug_group("conf",sprintf("welcome to circos v%s %s",$VERSION,$VERSION_DATE));
	...
	if ( $OPT{config} ) {
		...
		if ($cfile) {
	    printdebug_group("conf","loading configuration from file",$cfile);
		} else {
	    printdebug_group("conf","guessing configuration file");
	...
	my $outputfile = sprintf("%s/%s",
													 $CONF{image}{dir},
													 (fileparse($CONF{image}{file},qr/\.(png|svg)/i))[0]);
	...
	$CONF{chromosomes_units} = unit_convert(from=>$CONF{chromosomes_units},to=>'b');
	...
	if (debug_or_group("ideogram")) {
		my $max_label_len = max ( map { length($_->{chr}) } @IDEOGRAMS );
		my $max_len       = max ( map { length($_->{set}->max) } @IDEOGRAMS );
		my $fs = "%${max_label_len}s";
		my $fd = "%${max_len}d";
		for my $ideogram (sort {$a->{display_idx} <=> $b->{display_idx}} @IDEOGRAMS) {
	    printdebug_group("ideogram",
											 sprintf(
															 "%2d %2d $fs $fs %s $fd $fd %s $fd z $fd $fd s %.2f r %d rad %d %d %d %d %d prev $fs $fs next $fs $fs",
															 $ideogram->{idx},
	...
	for my $track ( @highlight_tracks ) {
		...
		$track->{__data}  = read_data_file($track_file,$track_type,$track_file_param);
	...
	my $link_names;
	my @link_tracks = map { parse_parameters($_, "link", 0) } 
    Circos::Track::make_tracks( fetch_conf("links","link"), 
																$track_default->{links},
																"link" );
	...
	for my $track ( @link_tracks ) {
		...
		$track->{__data} = read_data_file($track_file,"link",
																			{ addset       => 1,
																				minsize      => seek_parameter( 'minsize', @param_path ),
																				padding      => seek_parameter( 'padding', @param_path ),
																				record_limit => seek_parameter( 'record_limit', @param_path )
	...
	LINK:
	    ...
	    if ( seek_parameter( "ribbon", @i_param_path_link ) ) {
				...
				ribbon(mapoptions => {url=> $url},
							 ...
							 edgestroke => seek_parameter("stroke_thickness", @i_param_path_link),
				...
				if ( $num_bezier_control_points == 10 && $SVG_MAKE ) {
					...
					my $point_string = 
						join( $SPACE, map { sprintf( "%.1f", $_ ) } @bezier_control_points[ 2 .. $num_bezier_control_points - 1 ] );
					$svg = sprintf(
	...
	my @plot_tracks = map { parse_parameters($_, "plot", 0) } 
    Circos::Track::make_tracks( fetch_conf("plots","plot"),
																$track_default->{plots} );
	...
	for my $track ( @plot_tracks ) {
		...
		$track->{__data} = read_data_file($track_file, $track_type, $track_file_param, $track);
	...
	for my $track ( sort {$a->{z} <=> $b->{z}} @plot_tracks ) {
	    ...
	    if (my $bg = seek_parameter("backgrounds",@param_path)) {
				...
				for my $division (@$divisions) {
					...
					if (defined $division->{y0} && defined $division->{y1}) {
						my $radius1f   = ($division->{y0}-$plot_min)/($plot_max-$plot_min);
						my $radius2f   = ($division->{y1}-$plot_min)/($plot_max-$plot_min);
	    ...
	    if (my $axes = seek_parameter("axes",@param_path)) {
				...
				for my $division (@$divisions) {
					...
					my $radiusf   = ($division->{pos}-$plot_min)/($plot_max-$plot_min);
					my $radius;
					if ($orientation_direction == 1) {
    ...
    for my $datum_idx ( 0..@sorted_track_data-1 ) {
			...
			if ( $track_type eq "connector" ) {
				...
				my @dims = split( $COMMA,	seek_parameter( "connector_dims", @param_path ) );
			...
			if ( $track_type eq "text" ) {
				...
				$url = format_url(url=>$url,param_path=>[$data_point,$datum,@param_path]);
				Circos::Text::draw_text(%$labeldata); # ,mapoptions => { url=>$url } );
			...
			if ( $track_type eq "scatter" ) {
				...
				if ( $glyph eq "circle" ) {
					...
					my $angle_shift = seek_parameter( "angle_shift", $datum, @param_path ) || 0;
			...
			if ( $track_type eq "line" || $track_type eq "histogram" ) {
				...
				my ($line_brush, $line_colors) = fetch_brush( $thickness, $thickness ) if $thickness;
				...
				if ( $track_type eq "line" ) {
					...
					if ( ! defined $data_point_next || $data_point->{ideogram_idx} != $data_point_next->{ideogram_idx}) {
						$last_in_series = 1;
					}
					...
					if (defined_and_zero($extend_bin)) {
 ...
 OUT:
	...
	if ($PNG_MAKE) {
		if ( $CONF{tagname} ) {
			printinfo("tagging with $outputfile_png");
			$IM->stringFT(
										$COLORS->{black},
										locate_file(
																file =>
																$CONF{fonts}{ $CONF{ideogram}{label_font} || fetch_conf("default_font") || "default" },
																name=>"ideogram label font file",
															 ),
	...
	my $timer_max = 30;
	if (get_timer("circos") > $timer_max) {
		# force timer reporting
		if (! debug_or_group("timer")) {
			debug_group_add("timer");
			report_timer();
			printdebug_group("summary,timer","image took more than $timer_max s to generate. Components timings are shown above. To always show them, use -debug_group timer");
		}
	}
      ...
      default => [qw(
		init_counter
                                    ...
                                    increment_counter file
										...
										plot      => [qw( start end file min_size
...
sub read_data_file {
 ...
 LINE:
				...
				if ( $options->{param} ) {
					for my $param ( keys %{ $options->{param} } ) {
						...
						if ($param eq "fill_color") {
							@param_values = color_to_list( $value );
						} else {
	...
	for my $point (@$data) {
		if($point->{param}) {
			$point->{param} = parse_parameters($point->{param},$param_type);
		}
	}
		return $data;
}
...
sub is_on_ideogram {
	...
	for my $point ( @{$datum->{data}}) {
		my $chr       = $point->{chr};
		my $ideograms = get_ideograms_by_name($chr);
